...
void ofApp::draw(){
	...
	static std::vector<ofRectangle> currentRectangles;
    ...
    float barHeight = (float)fboHeight / numBands;
	...
	for (int i = 0; i < numBands; i++) {
		for (int j = 0; j < numBands; j++) {
			...
			ofColor myColor;
			myColor.setHsb(ofMap(bands[i] * bands[j], 0, 1, 0, 150), ofMap(bands[j], 0, 1, 10, 122), 200 * (bands [i] * bands [j]));
			// ofNoFill();
			// ofSetColor(255);
			// ofSetLineWidth(10);
			// ofDrawRectangle(bands[i] * i * barWidth * 4, bands[j] * j * barHeight * 4, barWidth, barHeight);
			ofSetColor(myColor);
			ofFill();
			ofSetLineWidth(5);
			ofDrawCircle(bands[i] * fboWidth, bands[j] * fboHeight, log(i) * j);
			ofDrawRectangle(bands[i] * i * barWidth * 4, bands[j] * j * barHeight * 4, barWidth, barHeight);
